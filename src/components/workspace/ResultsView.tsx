      ...
      <div className={`grid grid-cols-1 ${hideHeader ? 'gap-6' : 'lg:grid-cols-3 gap-6'}`}>
        <div className={`${hideHeader ? 'space-y-6' : 'lg:col-span-2 space-y-6'}`}>
          <section className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <p className="text-xl leading-relaxed text-white font-semibold text-wrap-pretty italic">
        ...
        <div className="space-y-6">
              ...
              {topics?.length > 0 ? topics.map((t: TopicItem, i: number) => (
                <div key={i} className="group">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-bold text-white tracking-tight">{t.topic}</span>
          ...
          {wordcloud_url && (
            <section className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
              ...
              <Image src={wordcloud_url} alt="Word Cloud" width={400} height={225} className="w-full h-full opacity-90 transition-opacity hover:opacity-100" unoptimized />
            </section>
          )}
        </div>
      </div>
